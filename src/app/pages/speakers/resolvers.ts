...
export function injectActiveSpeakers(): Speaker[] {
  ...
  let speakers = injectContentFiles<Speaker>((contentFile) =>
    ...
    .map((speaker) => {
      ...
      speaker.agenda = [...agenda[0].events, ...agenda[1].events].find(
        (e) => e.slug === speaker.talk.slug && e.slug,
      );
      return speaker;
    });
  ...
  for (let index = 0; index < 16 - speakerLenght; index++) {
    speakers.push({
      ...
      talk: { description: '...', title: '...', slug: '' },
      visible: true,
    });
  }

  return speakers.sort((a, b) => {
    let fa = a?.agenda?.startTime || '',
      fb = b?.agenda?.startTime || '';
      ...
      if (event.type.includes('Talk') || event.type.includes('Keynote')) {
        speakers.forEach((speaker) => {
          ...
          if (event.slug == speaker.talk.slug) {
            ...
            if (!event.title) event.title = speaker.talk.title || '';
            if (!event.description)
              event.description = speaker.talk.description || '';
