import { defineEventHandler } from 'h3';
import { TicketInterface } from 'src/app/models/ticket.interface';

export default defineEventHandler(() => tickets);

enum features {
  ConfAccess = 'In-person access to the conference',
  ConfCoffeeLunch = 'Coffee and Lunch included',
  Tshirt = 'NGRome T-shirt',
  Workshop = 'In-person Workshop on June 26',
  SpeakerDinner = 'Speaker Dinner on June 26',
  LunchBothDays = 'Lunch included both days',
  WorkshopLunch = 'Lunch included on 26',
}

const tickets: TicketInterface[] = [
  {
    visible: true,
    name: 'NGRome Conference - Student Ticket',
    subtitle:
      'Whether you are a beginner, intermediate, or expert, this will be the best opportunity',
    price: 50,
    realPrice: 155,
    features: [features.ConfAccess, features.ConfCoffeeLunch, features.Tshirt],
    event: 'ngrome-events/ngrome-conf-mmxxiv',
    ticket: 'fpdqo4sk5fw',
    saleStart: '1701388800000',
    saleEnd: '1701388800000',
  },
  {
    visible: true,
    name: 'NGRome Conference Blind Ticket',
    subtitle: 'Get your ticket now, ticket price will increase soon!',
    price: '75',
    realPrice: '155',
    features: [features.ConfAccess, features.ConfCoffeeLunch, features.Tshirt],
    event: 'ngrome-events/ngrome-conf-mmxxiv',
    ticket: 'blind-ticket',
    soldOut: true,
    saleStart: '1701388800000',
    saleEnd: '1701388800000',
  },
  {
    visible: true,
    name: 'Combo Tickets - Early Bird',
    subtitle: 'Workshop & Conference best value ticket',
    price: '325',
    popular: true,
    realPrice: '465',
    features: [
      features.Workshop,
      features.ConfAccess,
      features.WorkshopLunch,
      features.ConfCoffeeLunch,
      features.Tshirt,
    ],
    event: 'ngrome-events/modern-angular-architecture',
    ticket: '6pv0sbo8z5a',
  },
  {
    visible: true,
    name: 'Workshop - Early Bird ',
    subtitle: 'Modern Angular Architectures Workshop',
    price: 250,
    realPrice: 310,
    features: [features.Workshop, features.WorkshopLunch],
    event: 'ngrome-events/modern-angular-architecture',
    ticket: 'u6au8pc1wey',
  },

  {
    visible: true,
    name: 'VIP - Early Bird',
    subtitle: 'The best experience for your journey in Rome',
    bestExperience: true,
    price: '375',
    realPrice: '500',
    features: [
      features.Workshop,
      features.SpeakerDinner,
      features.ConfAccess,
      features.WorkshopLunch,
      features.ConfCoffeeLunch,
      features.Tshirt,
    ],
    event: 'ngrome-events/ngrome-conf-mmxxiv',
    ticket: 'sttgjfic3ma',
  },
  {
    visible: true,
    name: 'NGRome Conference - Early Bird ',
    subtitle: 'Get your ticket now, ticket price will increase soon!',
    price: 110,
    realPrice: 155,
    features: [features.ConfAccess, features.ConfCoffeeLunch, features.Tshirt],
  },
  {
    visible: false,
    name: 'NGRome Conference - Regular ',
    subtitle: 'Get your ticket now, ticket price will increase soon!',
    price: '155',
    features: [
      'Access to the conference',
      'Coffee and Lunch included',
      'NGRome T-shirt',
    ],
  },
  {
    visible: false,
    name: 'Workshop - Regular ',
    subtitle: 'Modern Angular Architectures Workshop',
    price: '310',
    features: ['Workshop on June 26', 'Lunch included on 26'],
  },
  {
    visible: false,
    name: 'Combo Tickets - Regular',
    subtitle: 'Get your ticket now, ticket price will increase soon!',
    price: '465',
    features: [
      'Workshop on June 26',
      'Conference on June 27',
      'Lunch included',
      'NGRome T-shirt',
    ],
  },
  {
    visible: false,
    name: 'VIP  - Regular Combo',
    subtitle: 'The best experience for your journey in Rome',
    price: '500',
    features: [
      'Workshop on June 26',
      'Speaker Dinner on June 26',
      'Conference on June 27',
      'Lunch included both days',
      'NGRome T-shirt',
    ],
  },
  {
    visible: false,
    name: 'NGRome Conference - Last Minute',
    subtitle: 'Get your ticket now!',
    price: '250',
    features: [
      'Access to the conference',
      'Coffee and Lunch included',
      'NGRome T-shirt',
    ],
  },
  {
    visible: false,
    name: 'Workshop - Last Minute ',
    subtitle: 'Modern Angular Architectures Workshop',
    price: '560',
    features: ['Workshop on June 26', 'Lunch included'],
  },
];
